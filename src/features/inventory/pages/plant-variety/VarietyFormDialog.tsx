    ...
    <DialogContent className="max-w-lg max-h-[85vh] overflow-y-auto">
      ...
      <div className="space-y-4 py-2">
        ...
        <div className="space-y-1.5">
          <Label>Species *</Label>
          <Select
            value={view.form.form.speciesId}
            onValueChange={(v) => view.form.updateField("speciesId", v)}
          >
            <SelectTrigger>
          ...
          {view.form.errors.speciesId && (
            <p className="text-xs text-destructive">{view.form.errors.speciesId}</p>
          )}
        </div>
        <div className="space-y-1.5">
          <Label>Variety Code</Label>
          <Input
            value={view.form.form.varietyCode}
            onChange={(e) =>
              view.form.updateField("varietyCode", e.target.value)
            }
            placeholder="Auto-generated if left blank"
          />
          {view.form.errors.varietyCode && (
            <p className="text-xs text-destructive">{view.form.errors.varietyCode}</p>
          )}
        </div>
        <div className="space-y-1.5">
      ...
      <DialogFooter>
        ...
        <Button
          ...
          disabled={!view.form.form.name || !view.form.form.speciesId || view.form.isSubmitting}
        >
